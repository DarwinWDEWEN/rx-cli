...
**定位**：在 Orca（Electron 桌面端 AI 协作编码工具）之上叠加团队协作模块。当前处于 **M1 后端收口 + M3 Harness 基础提前完成** 的阶段——进度呈"跳跃式"，不是按 M1→M2→M3 线性推进。

```mermaid
flowchart LR
    M1[M1 Teams+项目+IssuesPRs<br/>后端~90% / 前端 0%] --> M2[M2 Issue驱动+Worktree<br/>0%]
...
**当前测试基线**：8 个测试文件 / 106 tests 全绿；`tsc --noEmit -p config/tsconfig.node.json` 通过；协作库 `SCHEMA_VERSION = 5`。

## 2. 里程碑进度对照

| 里程碑 | 范围（ROADMAP §2） | 当前状态 | 缺口 |
|--------|--------------------|----------|------|
| M1 Teams + 项目接入 + IssuesPRs 基础 | 第 1-3 周 | 后端完成大部分，前端未开始 | B2/B6/B7/A5 后端；C1-C9 全部前端 |
...
| **C 系列前端依赖 B8（已完成）** | 前端随时可开工 | C1-C5 是当前性价比最高的下一步 |
...
| M1 交付（前端） | C1-C5（activeView + Sidebar + Teams 页 + IssuesPRs 页骨架） | 2 |
| M1 交付（前端） | C6-C9（项目接入引导 + 项目团队 UI + Issue/PR 列表详情） | 2 |
...
**合计：约 18-20 个 Round 剩余**（约等于 ROADMAP 剩余 7-8 周规划量）。估算为相对粒度，实际受任务大小影响。

## 8. 建议的下一步序列（当前建议）

```
Round 8:   C1-C2  activeView 扩展 + 导航历史（前端基础）
Round 9:   C3-C5  Sidebar 入口 + Teams 页 + IssuesPRs 页骨架
Round 10:  E1     Pipeline CLI 最小版（解锁 E5，打通 Agent 真实执行链）
Round 11:  D1+B7  生命周期引擎 + git-ref 注册器（解锁 worktree 链路）
```
